...
Original file is located at
    https://colab.research.google.com/drive/18hKwV2Lhs_yqFLKotPtJjEQsCXpAKCPz
"""
...
class FeedForwardNN:
    def __init__(self, hidden_layer_size, output_layer_size, input_layer_size, learning_rate, num_epochs, optimisation,
                 ...
                 factor=0.5, patience=3, dropout_probability=0):
        np.random.seed(0)
...
class RBMGibbsSampling:
    def __init__(self, num_visible_nodes, num_hidden_nodes, k, r, learning_rate, num_epochs):
...
def train(num_visible_nodes, num_hidden_nodes, k, r, learning_rate, num_epochs):
    X_train, y_train, X_val, y_val, X_test, y_test = load_dataset()
    rbm = RBMGibbsSampling(num_visible_nodes=num_visible_nodes, num_hidden_nodes=num_hidden_nodes, k=k, r=r,
                           learning_rate=learning_rate, num_epochs=num_epochs)
    for _ in range(rbm.num_epochs):
        (X_train, y_train) = shuffle(X_train, y_train, random_state=0)
        for x in X_train:
            x = x.reshape((1, 784))
            v = np.zeros((1, rbm.num_visible_nodes))
            ...
            for t in range(0, rbm.k + rbm.r):
                h = rbm.sample_h(v)
                v = rbm.sample_v(h)
                if t >= rbm.k:
                    act = rbm.activation((v @ rbm.W) + rbm.c)
                    update_W = update_W + (v.T @ act)
                    update_b = update_b + v
                    update_c = update_c + act
            temp = rbm.activation((x @ rbm.W) + rbm.c)
            update_W = (x.T @ temp) - (update_W / rbm.r)
            update_b = x - (update_b / rbm.r)
            update_c = temp - (update_c / rbm.r)
            rbm.update_parameters(update_W, update_b, update_c)
        classifier_input = rbm.sample_h(X_val)
        test_input = rbm.sample_h(X_test)
        classifier = FeedForwardNN(hidden_layer_size=rbm.num_hidden_nodes, output_layer_size=10, input_layer_size=rbm.num_hidden_nodes,
                                   learning_rate=0.001, num_epochs=10, optimisation="rmsprop", batch_size=32,
                                    activation_function="relu")
        classifier.fit(classifier_input, y_val, test_input, y_test,0)


sweep_config = {"method": "grid", "parameters": {
    "num_hidden_nodes": {"values": [256]},
...
def run_sweep():
    ...
    config = wandb.config
    train(num_hidden_nodes=config.num_hidden_nodes, num_visible_nodes=config.num_visible_nodes,
          learning_rate=config.learning_rate, num_epochs=config.num_epochs, k=config.k, r=config.r)

np.random.seed(0)
sweep_id = wandb.sweep(sweep_config, project="assignment_4_i")
wandb.agent(sweep_id , function=run_sweep ,  project="assignment_4_i" )